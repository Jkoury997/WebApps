...
const getUsersByEmpresa = async (req, res) => {
    try {
        const users = await userService.getUsersByEmpresa(req.params.empresaId);
        if (users.length === 0) {
            return res.status(404).json({ message: 'No se encontraron usuarios para esta empresa.' });
        }
        res.status(200).json(users);
    } catch (error) {
        res.status(500).json({ message: 'Error al obtener los usuarios por empresa', error: error.message });
    }
};
