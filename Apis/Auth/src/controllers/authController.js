...
const register = async (req, res) => {
    try {
        ...
        // Asegurarse de que el campo empresa esté presente
        if (!userData.empresa) {
            return res.status(400).json({ message: 'El campo empresa es obligatorio.' });
        }
...
const login = async (req, res) => {
    try {
        ...
        if (!email || !password || !empresa) {
            return res.status(400).json({ message: 'Todos los campos son obligatorios: email, contraseña y empresa.' });
        }

        // Llamar al servicio de login y obtener los tokens y el usuario
        const { accessToken, refreshToken, user } = await authService.loginUser(email, password, empresa);
