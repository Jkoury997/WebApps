...
const generateAccessToken = (user) => {
    return jwt.sign({ userId: user._id, empresa: user.empresa }, ACCESS_TOKEN_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRATION });
}

// Generar Refresh Token (vencimiento largo)
const generateRefreshToken = (user) => {
    return jwt.sign({ userId: user._id, empresa: user.empresa }, REFRESH_TOKEN_SECRET, { expiresIn: REFRESH_TOKEN_EXPIRATION });
}
