...
import { Info, Check, LogOut, MapPin, Tag, FileText, Calendar,ImageIcon } from 'lucide-react';
      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            {/* Título de la tarea */}
            <h3 className={`text-lg font-semibold ${tarea.completada ? "line-through text-muted-foreground" : ""}`}>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" size="icon" disabled={tarea.completada}>
                  {tarea.completada ? <Check className="w-4 h-4" /> : <LogOut className="w-4 h-4" />}
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
...
function CompleteTaskForm({ tareaId, handleSubmit, setNota, nota, handleImageChange }) {
    ...
    <form onSubmit={async (e) => {
      ...
      <div className="grid gap-2">
          <Label htmlFor="nota">Nota:</Label>
                    ...
                    <Textarea id="nota"
            value={nota} placeholder="Coloque un comentario."  onChange={(e) => setNota(e.target.value)} required/>
        </div>
        <div className="grid gap-2">
          <Label htmlFor="image">Subir imagen de la tarea completada:</Label>
          <Input id="image" type="file" accept="image/*" onChange={(e) => handleImageChangeInternal(e.target.files[0])} required />
         ...
         {imagePreview && (
         <div className="mt-2 w-full max-w-[450px]">  {/* Asegura que no exceda el tamaño máximo */}
...
function TaskDetails({ tarea }) {
  return (
    <div className="grid gap-4 py-4">
      <DetailItem icon={<Check />} label="Estado:" value={tarea.completada ? "Completada" : "Pendiente"} />
      ...
      {tarea.imagenAntes && (
  <div className="mt-2 w-full max-w-[450px]">  {/* Asegura que no exceda el tamaño máximo */}
    <Label className="font-semibold">Imagen del trabajo:</Label>
    <AspectRatio ratio={16 / 9} className="relative">
      <Image
