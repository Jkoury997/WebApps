...
// Función para filtrar tiendas excluidas
function filtrarVentaTotal(data) {
  const tiendasExcluir = [
    "Total",
  ];
...
// Función para filtrar y solo incluir las tiendas especificadas
function filtrarCajaVentaTotal(data) {
  const tiendasExcluir = [
    "Total",
  ];
...
export function calcularTotalesYPromedios(mergedData) {
 ...
 // Calcular promedios
 const promedioCAT = numTiendas > 0 ? totalUnidades / totalTickets : 0;
 const promedioPP = numTiendas > 0 ? totalBruto / totalUnidades : 0;
 const promedioTP = numTiendas > 0 ? totalBruto / totalTickets : 0;

 const promedioCATPrevio = numTiendas > 0 ? totalUnidadesPrevio / totalTicketsPrevio : 0;
 const promedioPPPrevio = numTiendas > 0 ? totalBrutoPrevio / totalUnidadesPrevio : 0;
 const promedioTPPrevio = numTiendas > 0 ? totalBrutoPrevio / totalTicketsPrevio : 0;
...
// Función para calcular totales y promedios
export function calcularTotalesYPromediosVentasTotales(mergedData) {
  // Filtrar las tiendas
  const MerdataTiendas = filtrarVentaTotal(mergedData);
...
// Función para calcular Cajas
export function calcularCajasTotalesVentasTotales(data) {

  // Filtrar las tiendas
  const dataTiendas = filtrarCajaVentaTotal(data);
...
// Función para filtrar tiendas excluidas
function filtrarIndividual(data, nombre) {
  if (!data || !nombre) return []; // Asegurarse de que los datos y el nombre están presentes

  const nombreTienda = nombre.toLowerCase(); // Convertir el nombre de la tienda a minúsculas

  return data.filter((item) => {
    const tiendaNombre = item.Tienda?.toLowerCase();
    return tiendaNombre === nombreTienda && Number(item.Bruto) !== 0;
  });
}


function filtrarCajasIndividual(data,nombre) {
  console.log(data)
  console.log(nombre)
  const tiendasIncluir = [
      nombre
...
// Función para calcular Cajas
export function calcularCajasIndividual(data,nombre) {
  // Filtrar las tiendas
  const dataTiendas = filtrarCajasIndividual(data,nombre);
  // Inicializar acumuladores
  let Total = 0;
  let Punto1 = 0;
  let Punto2 = 0;
...
// Función para calcular totales y promedios
export function calcularTotalesYPromediosIndividual(mergedData,nombre) {
  // Filtrar las tiendas
  const MerdataTiendas = filtrarIndividual(mergedData,nombre);

  console.log(MerdataTiendas)
