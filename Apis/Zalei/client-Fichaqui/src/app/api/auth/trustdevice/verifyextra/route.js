...
export async function POST(req) {
    try {
        ...
        const { localFingerprint,newFingerprint } = body;

        // Enviar la solicitud de registro del dispositivo de confianza al backend
        const response = await fetch(`${URL_API_AUTH}/api/trust-device/verifyExtra`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ userId, localFingerprint,newFingerprint })
        });
        ...
        if (response.ok) {
            ...
            cookieStore.set('fingerprint', newFingerprint, {
                path: '/'
